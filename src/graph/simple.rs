...
impl SimpleDFSGraphPath {
    fn new(list: Vec<Node>, point: usize) -> Self {
        ...
        let mut p = SimpleDFSGraphPath {
            point,
            marked: Box::into_raw(Box::new(marked)),
            path: Box::into_raw(Box::new(path)),
            list,
        };
    ...
    fn dfs(&self, point: usize) {
        unsafe {
            (&mut *self.marked).get_mut(point).map(|b| {
                if *b {
                    return;
                }
                *b = true;
            });
            if let Some(node) = self.list.get(point) {
                let mut len: usize = 0;
                while let Some(&n) = (&node.list).get(len) {
                    len += 1;
                    if n < 0 {
                        continue;
                    }
                    ...
                    (&mut *self.path).get_mut(n as usize).map(|p| {
                        *p = point as isize;
                    });
                    self.dfs(n as usize);
                }
...
impl Drop for SimpleDFSGraphPath {
    fn drop(&mut self) {
        unsafe {
            let path = Box::from_raw(self.path);
            let marked = Box::from_raw(self.marked);
